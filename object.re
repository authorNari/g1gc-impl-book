...
本章では@<code>{oopDesc}などのインスタンスを別名定義のルールに従って@<code>{oop}のように呼ぶことにします。
...
@<code>{Klass}クラスは名前のとおり、型情報を保持しています。
@<code>{Klass}のインスタンスは@<code>{klassOop}の一部として生成されます。

@<code>{Klass}はさまざまな型情報の抽象的な基底クラスです。
...
もし、仮想関数を使って子クラスごとに違う振る舞いをするメンバ関数を定義したい場合は@<code>{oopDesc}ではなく、対応する@<code>{Klass}の方に仮想関数を定義しなければなりません。
...
つまり、5行目では@<code>{1}というアドレス上のデータをインスタンスとみなして関数呼び出しているのですから、SEGVが起こってもおかしくないような…？
...
C++では関数呼び出しと同様の構文で明示的な型変換をおこなえます。
...
このメンバ関数は並列に動く可能性があり、途中でほかのスレッドに割り込まれて先にコピーされてしまった場合は@<code>{NULL}を返します。
...
G1GCやそのほかのコピーGCでは上記の@<code>{is_forwarded()}を確認し、マークが付いているものは再度コピーしないようにします。
