...
@<code>{oops_do()}内では自クラスが管理するオブジェクトに対する参照（ルート）（@<code>{oop}）を引数にして、@<code>{do_oop()}を呼び出しています。
...
次に2.ついて説明しましょう。
複数のスレッドから並列で実行される場合に、@<code>{process_strong_roots()}はルートスキャンのタスクを適当な大きさに分割して、各スレッドに早いもの勝ちでそれぞれのタスクを実行させることで並列実行時に性能がでるようにしています。
...
そうすることで「ルート探索にかかった時間」と、実際に「ルートをコピー（とマーク）した時間」を別々に計測でき、よりよいプロファイル情報を提供することができるというわけです。
...
『アルゴリズム編』で書いたくらいの情報がわかっていれば問題ないでしょう。
