...
//image[vm_heap_internal][オブジェクト管理機能内の全体像。@<code>{CollectedHeap}クラスがインタフェースとなる。@<code>{CollectedHeap}クラスは@<code>{CollectedPolicyクラス}内の設定値を参考に振る舞いを決定する。@<code>{CollectedHeap}クラスは各GCクラスに対してヒープ内のGCを要求する。]

まず、登場人物を簡単に説明しましょう。
@<code>{CollectedHeap}というクラスはオブジェクトを割り当てるVMヒープを管理します。
また、オブジェクト管理機能のインタフェースとして機能し、@<code>{CollectedPolicy}クラスの情報を参照して適切な処理を実行します。

@<code>{CollectedPolicy}クラスはオブジェクト管理機能全体に振る舞いを定義するクラスです。
...
@<img>{vm_heap_internal}のように、VMからオブジェクト割り当て要求が行われた場合は、まず@<code>{CollectedHeap}クラスが要求を受け、@<code>{CollectedPolicy}クラスの方針にしたがって、内部のメモリ空間にあるオブジェクトを割り当てます。
