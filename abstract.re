...
1.は、VMがオブジェクトの種類を指定すると、VMヒープの内部に割り当てたオブジェクトの実体が帰えしてくれるインタフェースです。

2.は、GC実行要求を出すと内部でGCを実行してくれるインタフェースです。

VMはVMヒープ内のオブジェクトの位置や形状がわかりません。そのため、3.が必要になります。
具体的には、VMヒープ内の全オブジェクトに指定した関数を適用する、また、あるオブジェクト内の全フィールドに指定した関数を適用する、あるポインタが割り当てられたオブジェクトか確認する、といったインタフェース群が定義されています。

上記のインタフェースさえ守れていれば、オブジェクト管理機能の内部実装は好きに変更できます。
インタフェースが同じであれば異なるGCアルゴリズムを実装することも可能なわけです。

VMは基本的に上記のインタフェースを利用して実装されますが、例外的にオブジェクト管理機能の内部実装を意識する部分もあります。
