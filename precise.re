...
実際のスタックマップは@<code>{00100}というようなビット列です。
このスタックマップは、ビット列のビットが立っている部分に対応するローカル変数（またはオペランドスタック）に参照型の値が格納されていることを示しています。
...
そして、スタックマップは抽象的インタプリタが記録したバイトコードに対応する型情報から作成されます。
...
GCは、このスタックマップを見て、ローカル変数0とオペランドスタックの先頭に参照型の値が格納されていると判断し、それらが参照するオブジェクトはGCに確実に「生きている」と判断されます。
...
ただ、それをやってしまうとマップが膨大な量になってしまうため、基本的に以下の4つのタイミングのマップのみを生成します。

 1. 後方分岐（例: ループで後ろにジャンプするとか）
 2. メソッド呼び出し
 3. return
