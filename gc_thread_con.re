...
具体的な問題としてJITの話を持ちだしてみましょう。
HotspotVMでコンパイル済みのコードを実行する際に、スタックやレジスタのどの部分がオブジェクトへの参照であるかを示す@<b>{GCマップ}を更新しています。
そして、GCはこのGCマップを参考にルートを列挙するわけです。
GCマップの更新はコストがかかるので、定期的にしか実行できません。
そのため、セーフポイントとしてスレッドを停止するタイミングは、GCマップを更新した直後、つまりGCマップが最新の状態でなければなりません。
なぜならGCマップが古いと存在するはずの参照を取りこぼしている可能性があり、すべてのルートを列挙するという目的を達成できないのです。
GCマップの詳細については、@<chap>{precise}でまた詳しく説明します。
