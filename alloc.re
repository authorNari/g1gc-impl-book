...
1810行目の@<code>{collector_policy()}メンバ関数はG1GCに関するフラグや設定値などが定義されている@<code>{G1CollectorPolicy}インスタンスへのポインタを返し、@<code>{max_heap_byte_size()}メンバ関数は名前のとおり、最大G1GCヒープサイズを返します。
したがって、@<code>{max_byte_size}ローカル変数には最大G1GCヒープサイズが格納されます。

1819行目の@<code>{pgs}にはパーマネント領域に関する設定値などが定義されている@<code>{PermanentGenerationSpec}が格納されます。
...
実装方法は拍子抜けするほどシンプルです。
具体的には次の手順で処理を行います。
ここでは説明を簡単にするため、アライメントサイズ（リージョンサイズ）を1Kバイト、VMヒープのサイズは1Kバイトよりも大きいと仮定します。
...
メモリ領域の予約には@<code>{os::reserve_memopry()}関数を使います。
...
メモリ領域の予約を実際におこなう@<code>{mmap()}や@<code>{VirtualAlloc()}は予約するメモリ領域の先頭アドレスを指定できますので、これを利用します。
