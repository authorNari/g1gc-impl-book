...
ミューテックスは排他制御の基本的な実装であり、ミューテックスを土台としてさまざまな排他制御が実装されます。
...
返し終わった客は待合室の客の内1人、もしくは客全員を呼ぶことが可能です。
...
そのため、@<img>{thread_park_unpark}のように、HotspotVMが管理する1スレッド（@<code>{Thread}インスタンス）の@<code>{_MutexEvent}に対して、@<code>{park()}・@<code>{unpark()}を呼ぶことで、対象のスレッドを一時停止・再起動させることが可能です。
...
また、客（スレッド）はモニタのしきたりに則って、どの店にも入店できます。
...
また、大域脱出の際（例外発生時など）にモニタのアンロックをし忘れる、といった凡ミスも防げます。
