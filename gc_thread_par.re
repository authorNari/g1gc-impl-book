...
まず、@<code>{AbstractWorkGang}は1つだけモニタを持っており、モニタの待合室には@<code>{AbstractWorkGang}に所属する@<code>{GangWorker}を待たせます（@<img>{work_gang_do_task_1}）。
...
上記のコードからはモニタの初期化とデータの初期化がおこなわれることがわかれば充分です。
...
また、246行目の@<code>{part}はワーカーの順番を記録するローカル変数です。
