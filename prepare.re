...
OpenJDKには複数のプロジェクトを持ち、それにともなって複数のリポジトリが存在します。
今回はMercurialを使ってHotspotVMプロジェクトのリポジトリからのみソースコードを入手しましょう。
...
@<code>{cpu}		CPU依存コード群
@<code>{os}		OS依存コード群
@<code>{os_cpu}		OS、CPU依存コード群（Linuxでかつx86等）
@<code>{share}		共通コード群
//}
...
おおまかに言って、HotspotVMには次のGCアルゴリズムが実装されています。
